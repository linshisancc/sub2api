...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	feishuWebhookService := service.ProvideFeishuWebhookService(settingRepository, redisClient)
	rateLimitService := service.ProvideRateLimitService(accountRepository, usageLogRepository, configConfig, geminiQuotaService, tempUnschedCache, timeoutCounterCache, openAI403CounterCache, settingService, compositeTokenCacheInvalidator, feishuWebhookService)
	...
	balanceNotifyService := service.ProvideBalanceNotifyService(emailService, settingRepository, accountRepository, feishuWebhookService)
	...
	accountWarmupService := service.ProvideAccountWarmupService(settingRepository, accountRepository, accountTestService, feishuWebhookService, redisClient, configConfig)
	settingHandler := handler.ProvideAdminSettingHandler(settingService, emailService, turnstileService, opsService, paymentConfigService, paymentService, userAttributeService, notificationEmailService, accountWarmupService, totpService, userService)
	...
	loginBruteforceService := service.NewLoginBruteforceService(settingRepository, redisClient, feishuWebhookService)
	securityHandler := admin.NewSecurityHandler(loginBruteforceService)
	upstreamBillingProbeService := service.ProvideUpstreamBillingProbeService(accountRepository, accountTestService, settingService, leaderLockCache, db)
	ollamaCloudUsageService := service.ProvideOllamaCloudUsageService(accountRepository, httpUpstream, settingService, secretEncryptor, configConfig, leaderLockCache, db)
	adminHandlers := handler.ProvideAdminHandlers(dashboardHandler, adminUserHandler, groupHandler, accountHandler, adminAnnouncementHandler, dataManagementHandler, backupHandler, oAuthHandler, openAIOAuthHandler, geminiOAuthHandler, antigravityOAuthHandler, grokOAuthHandler, proxyHandler, adminRedeemHandler, promoHandler, settingHandler, opsHandler, systemHandler, adminSubscriptionHandler, adminUsageHandler, userAttributeHandler, errorPassthroughHandler, tlsFingerprintProfileHandler, adminAPIKeyHandler, scheduledTestHandler, channelHandler, channelMonitorHandler, channelMonitorRequestTemplateHandler, contentModerationHandler, promptAdminHandler, paymentHandler, affiliateHandler, complianceHandler, auditLogHandler, securityHandler, upstreamBillingProbeService, ollamaCloudUsageService)
	...
	userAccountHandler := handler.NewUserAccountHandler(apiKeyService, accountRepository, accountUsageService)
	...
	ipBanCheckMiddleware := middleware.NewIPBanCheckMiddleware(loginBruteforceService)
	loginBruteforceTrackerMiddleware := middleware.NewLoginBruteforceTrackerMiddleware(loginBruteforceService)
	engine := server.ProvideRouter(configConfig, handlers, jwtAuthMiddleware, adminAuthMiddleware, apiKeyAuthMiddleware, auditLogMiddleware, stepUpAuthMiddleware, ipBanCheckMiddleware, loginBruteforceTrackerMiddleware, apiKeyService, subscriptionService, opsService, settingService, compositeRouteResolver, redisClient)
	httpServer := server.ProvideHTTPServer(configConfig, engine)
	opsMetricsCollector := service.ProvideOpsMetricsCollector(opsRepository, settingRepository, accountRepository, concurrencyService, db, redisClient, configConfig)
	opsAggregationService := service.ProvideOpsAggregationService(opsRepository, settingRepository, db, redisClient, configConfig)
	opsAlertEvaluatorService := service.ProvideOpsAlertEvaluatorService(opsService, opsRepository, emailService, feishuWebhookService, redisClient, configConfig, proxyRepository)
