...
// GroupWindowStatsWithName holds per-group 5h/7d aggregated usage for admin view.
type GroupWindowStatsWithName struct {
...
// GetAllGroupsWindowStats returns 5h/7d aggregated stats for all groups with recent usage.
func (s *DashboardService) GetAllGroupsWindowStats(ctx context.Context) ([]GroupWindowStatsWithName, error) {
	now := time.Now()
	start5h := now.Add(-5 * time.Hour)
	start7d := now.Add(-7 * 24 * time.Hour)

	stats5h, err := s.usageRepo.GetGroupStatsWithFilters(ctx, start5h, now, 0, 0, 0, 0, nil, nil, nil)
	if err != nil {
		return nil, fmt.Errorf("get all groups 5h stats: %w", err)
	}
	stats7d, err := s.usageRepo.GetGroupStatsWithFilters(ctx, start7d, now, 0, 0, 0, 0, nil, nil, nil)
	if err != nil {
		return nil, fmt.Errorf("get all groups 7d stats: %w", err)
	}

	type entry struct {
		name         string
		requests5h   int64
		actualCost5h float64
		requests7d   int64
		actualCost7d float64
	}
	m := make(map[int64]*entry)
	for _, s := range stats7d {
		e := &entry{name: s.GroupName, requests7d: s.Requests, actualCost7d: s.ActualCost}
		m[s.GroupID] = e
	}
	for _, s := range stats5h {
		if e, ok := m[s.GroupID]; ok {
			e.requests5h = s.Requests
			e.actualCost5h = s.ActualCost
		} else {
			m[s.GroupID] = &entry{name: s.GroupName, requests5h: s.Requests, actualCost5h: s.ActualCost}
		}
	}

	result := make([]GroupWindowStatsWithName, 0, len(m))
	for id, e := range m {
		result = append(result, GroupWindowStatsWithName{
			GroupID:      id,
			GroupName:    e.name,
			Requests5h:   e.requests5h,
			ActualCost5h: e.actualCost5h,
			Requests7d:   e.requests7d,
			ActualCost7d: e.actualCost7d,
		})
	}
	// Sort by 7d actual cost descending
	for i := 1; i < len(result); i++ {
		for j := i; j > 0 && result[j].ActualCost7d > result[j-1].ActualCost7d; j-- {
