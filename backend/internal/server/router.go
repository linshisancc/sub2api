...
func SetupRouter(
	...
	ipBanCheck middleware2.IPBanCheckMiddleware,
	loginBruteforceTracker middleware2.LoginBruteforceTrackerMiddleware,
	...
	// 全局 IP 封禁检查紧随安全客户端 IP 解析，命中后不再进入业务中间件和路由。
	r.Use(gin.HandlerFunc(ipBanCheck))
	...
	// 注册路由
	registerRoutes(r, handlers, jwtAuth, optionalJWTAuth, adminAuth, apiKeyAuth, auditLog, stepUpAuth, loginBruteforceTracker, apiKeyService, subscriptionService, opsService, settingService, compositeResolver, cfg, redisClient)
...
func registerRoutes(
	...
	// 注册各模块路由
	routes.RegisterAuthRoutes(v1, h, jwtAuth, auditLog, loginBruteforceTracker, redisClient, settingService, panelRateLimiter)
