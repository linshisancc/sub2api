...
> **注意**：上面是「典型」链路，但你的实际部署不一定长这样。反向代理可能是 Caddy、
> nginx，也可能是 Cloudflare Tunnel（cloudflared），甚至没有任何反代而由 Cloudflare
> 直连源站 80 端口。**排查前必须先确认真实链路结构**，见下方「第 0 步」。

---

## 第 0 步：先确认链路实际结构

不要假设链路里有 Caddy。先在服务器上确认每一层是否真实存在，再决定排查路径：

```bash
# 1. 容器与端口映射（确认 sub2api 监听在宿主机哪个端口）
docker ps -a

# 2. 是否有反向代理 / 隧道进程
which caddy cloudflared
ps aux | grep -E 'caddy|cloudflared|nginx' | grep -v grep
...
| 现象 | 实际链路 | 排查重点 |
|------|----------|----------|
| 有 caddy 进程 | CF → Caddy → sub2api | 查 Caddy `encode` 压缩（第 2 层） |
| 有 cloudflared 进程 | CF → Tunnel → sub2api | 无 Caddy 层，直接查 Cloudflare（第 3 层） |
| 无反代进程，仅 80 端口被 docker-proxy 占 | CF → 源站:80 直连 sub2api | 无 Caddy 层，唯一代理是 Cloudflare（第 3 层） |

> 若链路里没有 Caddy，则本仓库的 `deploy/Caddyfile` **从未被加载**，改它毫无意义。
...
排查前先避开这几个常见误区，否则会得出错误结论：

- **prompt 不能太短**：`数到20` 这类内容模型会塞进**单个** `content_block_delta` 事件，
  哪怕链路完美也是「一次出现」，看不出流式节奏。要用会产生**大量 delta、持续数秒**
  的长 prompt，例如「用中文写一篇约 300 字的短文，慢慢写」。
- **必须实时盯终端**：要判断「一块一块」，得**眼睛看着终端字符怎么蹦出来**，
  而不是看 `curl` 跑完后的完整输出 —— 完整输出看不出事件到达的时间节奏。
- **多 token 的 delta 是正常的**：每个 `content_block_delta` 一次带一小段词（而非单字）
  是上游 API 的正常行为，不是攒块。真正的故障表现是「卡住数秒后几十上百字猛地涌出」。
- **curl 命令别多行粘贴**：多行命令粘进终端易被折行损坏，导致 `-d` 失效、请求退化成
  GET（后端会返回 `404 page not found`，造成「接口不存在」的假象）。把请求体写进文件、
  用 `-d @req.json` 最稳：
  ```bash
  cat > /tmp/req.json <<'EOF'
  {"model":"claude-opus-4-7","max_tokens":512,"stream":true,"messages":[{"role":"user","content":"用中文写一篇约300字的短文,慢慢写"}]}
  EOF
  ```
...
```bash
# A. 经完整链路（CLI → Cloudflare → ... → 后端）
curl -N -sS -D - https://api.你的域名/v1/messages \
  -H 'content-type: application/json' \
  -H 'x-api-key: <你的key>' \
  -d @/tmp/req.json

# B. 绕过 Cloudflare，直连源站 IP
curl -N -sS -D - https://api.你的域名/v1/messages \
...
# C. 在服务器本机直打后端容器，绕过所有代理（端口换成 docker ps 看到的实际端口）
curl -N -sS -D - http://127.0.0.1:8080/v1/messages \
  -H 'content-type: application/json' \
  -H 'x-api-key: <你的key>' \
  -d @/tmp/req.json
```

> **`--resolve` 没绕过 CF 的判别**：若 B 的响应头里仍出现 `server: cloudflare` 或
> `cf-ray`，说明根本没绕过 —— 多半是 `<源站公网IP>` 填成了 Cloudflare 的 IP（用
> `dig` 解析橙云域名得到的就是 CF 的 IP）。要填**源站自己的公网 IP**，在源站执行
> `curl -4 ifconfig.me` 获取。
...
- [ ] 第 0 步：先确认链路真实结构（有无 Caddy / cloudflared / nginx），别假设有 Caddy。
- [ ] 测试：用长 prompt + 实时盯终端，请求体写文件用 `-d @req.json`。
