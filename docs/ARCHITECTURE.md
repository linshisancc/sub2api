...
- 分组内须有 **2 个及以上同平台账号**，才能在限流/故障时自动切换。单账号分组在 429 后无可切换目标，请求直接返回错误（503 会原地重试最多 3 次后失败）。
- 跨平台账号（如 OpenAI + Claude）**不支持**放在同一分组内互相切换，分组的 `platform` 字段固定决定请求转发协议，平台不同的账号在选号时会被直接过滤。

**多账号负载分摊行为**

同一分组内有多个同平台账号时，请求按以下规则分配：
