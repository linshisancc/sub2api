...
发起一条极小的请求，把第一个 5h 窗口的起点钉在 08:00；之后系统会按 5 小时间隔自动续
触发后续窗口，无需等待客户端真实请求。这样工作日就能用满
...
| 维度 | 行为 |
|------|------|
| **触发时间** | 默认每天 08:00 起，每 5 小时自动触发一次后续窗口；通过 5 段 cron 表达式可配置首个窗口锚点（如 `15 7 * * *` = 每天 07:15） |
| **触发条件** | `enabled=true` + 窗口边界命中 + （可选）当日为工作日 + 当前窗口未执行过 |
| **覆盖账号** | 所有 `schedulable=true` 且未被限流 / 过期 / 临时不可调度的账号，按平台白名单过滤 |
| **并发度** | 最多 10 个账号并行探针（信号量） |
| **幂等** | 同一个 5h 窗口只执行一次；08:00 已执行不会阻挡 13:00 / 18:00 等后续窗口 |
...
2. **解析 `scheduled_warmup_cron`**，把 cron 命中的当日首个时间作为窗口锚点，并按 5 小
   时推导后续窗口；判断 `now` 是否处于「上次 tick → now」区间的某个窗口边界。若否，返回。
