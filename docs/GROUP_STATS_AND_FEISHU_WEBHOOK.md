...
把首个 5h 窗口起点钉在 08:00；之后在 8 小时覆盖范围内每 5 小时自动触发下一轮窗口
（默认只需要 13:00），无需等待客户端真实请求。
...
|-------|------|--------|
| 启用 | 总开关 | 关闭 |
| 触发时间（5 段 cron） | 标准 cron 表达式，用于配置当天首个 5h 窗口锚点；后续窗口在 8 小时覆盖范围内每 5 小时自动触发 | `0 8 * * *`（每天 08:00） |
| 仅工作日触发 | 是否限定在工作日 | 开启 |
...
2. 解析 `scheduled_warmup_cron`，以当天 cron 命中的首个时间为锚点，在 8 小时覆盖范围内推导 5h 窗口边界，判断 `now` 是否处于"上次 tick → now"区间的窗口边界；若否，返回。
...
5. 申请 Redis 分布式锁（key=`scheduled_warmup:leader`，TTL=15 分钟，多副本部署下只允许一个副本执行）。
